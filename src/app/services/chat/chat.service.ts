...
export class ChatService implements OnDestroy {
  ...
  private subject = new BehaviorSubject<{
    [index: string]: { [index: number]: ChatMessage };
  }>({});
  public data$ = this.subject.asObservable();
  private countSubject = new BehaviorSubject<{ [key: string]: number }>({
    _all: 0,
  });
  ...
  constructor(
    ...
    ws.on<{ messages: ChatMessage[] }>('chat')
      .pipe(
        map(({ messages }) => {
          ...
          messages.map((message) => {
            ...
            if (this.login === from) {
              chats[to] = {
                ...chats[to],
                ...{ [timestamp]: { ...message, img: this.getImg(from) } },
              };
            }
            if (this.login === to) {
              chats[from] = {
  ...
  private getImg(login: string): Observable<string> {
    return this.userInfo.data$.pipe(
      map((userData) => {
        ...
        return data?.photo.paths.find((src: string) =>
          src.includes(data?.photo.profilePhoto)
        );
      })
    );
  }
