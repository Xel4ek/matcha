...
export class FirstAccessComponent implements OnDestroy {
  ...
  updatePhoto(photoInfo: { [key: string]: string }): void {
    if (this.profile) {
      ...
      if (removePhoto) {
        this.profile.photo.paths = this.profile.photo.paths.filter(
          (currentPhoto: string) => currentPhoto !== removePhoto
        );
    ...
    if (this.profile && this.firstAccess) {
      ...
      photos?.paths.map((photo: string) => {
        this.ws.send('profile', {
          photo,
          isProfile: photo === photos.profilePhoto,
        });
      });
